...
class TouchTreeview(ttk.Treeview):
    ...
    def on_touch_scroll(self, event):
        ...
        if abs(dy) > 5:
            self._moved = True
            self.ignore_select = True  # suppress selection
        return "break"

    def on_touch_end(self, event):
        if not self._moved and self._tap_item:
            self.ignore_select = False  # allow selection
            self.selection_set(self._tap_item)
            self.focus(self._tap_item)
        else:
            self.ignore_select = True  # ignore selection
...
class ItemList:
    ...
    def get_dot_coords(self, n: int, t: float, c: int):
        ...
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]get dot coords error:[/red] {e}")
            return 0, 0, 0, 0
    ...
    def refresh_row_tags(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]refresh row tags error:[/red] {e}")
    ...
    def on_select(self, event):
        try:
            ...
            selected_item = self.listbox.focus()  # ID of selected element
            playable = True
            ...
            if self.errorlog is True: self.flexprint(f"[red]on select error:[/red] {e}")
    ...
    def do_close(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]do close error:[/red] {e}")
