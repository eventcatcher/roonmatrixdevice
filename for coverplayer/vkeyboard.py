...
class VirtualKeyboard:
    ...
    def circleProgress(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]circleProgress error:[/red] {e}")
    ...
    def get_dot_coords(self, n: int, t: float, c: int):
        ...
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]get dot coordss error:[/red] {e}")
            return 0, 0, 0, 0
    ...
    def update_keyboard(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]update keyboard error:[/red] {e}")
    ...
    def vpresskey(self, x):
        try:
            ...
            if self.debug is True: self.flexprint("The cursor is at: ", cursor_pos)
            #self.master.withdraw()
            actualValue = str(self.inpstr.get())
            x = self.buttonsTranslate[x] if x in self.buttonsTranslate else x
            if self.debug is True: self.flexprint('vpresskey x: ' + str(x) + ', buttonsTranslate: ' + str(self.buttonsTranslate))
            if x == 'lock':
                ...
                if x == 'enter':
                    ...
                    if cursor_pos > 0:
                        value = actualValue[0:cursor_pos]
                    if self.debug is True: self.flexprint('shift: ' + str(self.shift_key_pressed) + ', alt: ' + str(self.alt_key_pressed) + ', capslock: ' + str(self.capslock_key_enabled))
            ...
            if self.errorlog is True: self.flexprint(f"[red]vpresskey error:[/red] {e}")
    ...
    def shift_cursor_left(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]shift cursor left error:[/red] {e}")
    ...
    def shift_cursor_right(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]shift cursor right error:[/red] {e}")
    ...
    def vupdownkey(self, event, type):
        try:
            if type == 'shift':
                ...
                if self.debug is True: self.flexprint('shift_key_pressed: ' + str(self.shift_key_pressed))
                self.update_keyboard()
            if type == 'alt':
                self.alt_key_pressed = self.alt_key_pressed is False
                if self.debug is True: self.flexprint('alt_key_pressed: ' + str(self.alt_key_pressed))
                self.update_keyboard()
        except Exception as e:
            if self.errorlog is True: self.flexprint(f"[red]vupdownkey error:[/red] {e}")
    ...
    def get_next_playtype(self, searchtype):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]get next playtype error:[/red] {e}")
            return 'artist'
    ...
    def on_labelTap(self):
        try:
            ...
            if self.debug is True: self.flexprint('on_labelTap => s_type: ' + str(s_type) + ', zonetype: ' + str(self.zonetype) + ', stream_on: ' + str(self.stream_on))
            self.searchtype = s_type
            self.label.config(text=self.lang[self.searchtype])
        except Exception as e:
            if self.errorlog is True: self.flexprint(f"[red]on labelTap error:[/red] {e}")
    ...
    def start(self, type, data, keyb_list, lang, hasRadioSearch, zonetype, sourcetype, alternative_layout, kp_callback, close_callback):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]start error:[/red] {e}")
    ...
    def toggle_sourcetype(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]toggle sourcetype error:[/red] {e}")
    ...
    def engine(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]engine error:[/red] {e}")
    ...
    def error_message(self, message):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]error message error:[/red] {e}")
    ...
    def close(self):
        try:
            ...
            if self.errorlog is True: self.flexprint(f"[red]close error:[/red] {e}")
